...
//        if (true) throw new RuntimeException("FlutterBluetoothSerial Attached to activity");
                ...
                (requestCode, permissions, grantResults) -> {
                    switch (requestCode) {
                        case REQUEST_COARSE_LOCATION_PERMISSIONS:
                            // 修补: 可能同时请求多个权限, 需全部授予才算通过; 并防 null
    ...
    private void ensurePermissions(EnsurePermissionsCallback callbacks) {
        // 修补: Android 12(API 31)+ 读写蓝牙需运行时权限 BLUETOOTH_CONNECT / BLUETOOTH_SCAN,
        // 老插件只请求定位权限, 其回调里调 getBondedDevices() 会抛 SecurityException 直接崩进程。
            ...
            switch (call.method) {
                ...
                case "getBondedDevices":
                    ensurePermissions(granted -> {
                        if (!granted) {
                            result.error("no_permissions", "读取已配对设备需要蓝牙权限(BLUETOOTH_CONNECT)", null);
                            return;
                        }

                        // 修补: 权限被拒/被系统撤销时 getBondedDevices 会抛 SecurityException,
                        // 必须捕获并以错误返回, 否则在主线程崩溃。
                        try {
                            ...
                            result.error("security_exception", e.getMessage(), null);
                        }
                    });
                    break;
                ...
                case "startDiscovery":
                    ensurePermissions(granted -> {
                        ...
                        // 修补: 同样需捕获 SecurityException(BLUETOOTH_SCAN 被拒时)
                        try {
